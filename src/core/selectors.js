...
export function canExplore(){
  return state.flags[FLAGS.CAGE_BROKEN] && state.energy >= COST.EXPLORE && !inDialogue() && !inGameOver();
}

export function atCaveEntrance(){
  return state.area === 'cave' && state.room === 'entrance';
}
export function atPool(){
  return state.area === 'cave' && state.room === 'pool';
}
